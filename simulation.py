...
speeds = {'car':3, 'bus':2.22, 'truck':2.442, 'rickshaw':2.22, 'bike':3.33}  # car speed reduced by 5% (others unchanged)
...
def setTime():
    ...
    # Announce detection (only on platforms with `say` available)
    try:
        if os.name == 'posix':
            os.system("say detecting vehicles, "+directionNumbers[(currentGreen+1)%noOfSignals])
    except Exception:
        pass
    # Choose the next green signal as the direction with the most waiting vehicles.
    global nextGreen
    waiting = compute_waiting_counts()
    # Exclude current green lane from consideration
    waiting_for_choice = waiting[:]
    waiting_for_choice[currentGreen] = -1
    # Find index of maximum waiting vehicles
    max_wait = max(waiting_for_choice)
    if max_wait <= 0:
        # fallback to the next sequential lane
        chosen = (currentGreen+1) % noOfSignals
    else:
        # choose first lane with max waiting
        chosen = waiting_for_choice.index(max_wait)
    nextGreen = chosen
...
#       greenTime = defaultMaximum
    ...
    # Count vehicles waiting in the chosen lane to compute green time
    noOfCars, noOfBuses, noOfTrucks, noOfRickshaws, noOfBikes = 0,0,0,0,0
    chosen_dir = directionNumbers[nextGreen]
    for j in range(len(vehicles[chosen_dir][0])):
        vehicle = vehicles[chosen_dir][0][j]
        if getattr(vehicle, 'crossed', 0) == 0:
            noOfBikes += 1
    for i in range(1,3):
        for j in range(len(vehicles[chosen_dir][i])):
            vehicle = vehicles[chosen_dir][i][j]
            if getattr(vehicle, 'crossed', 0) == 0:
                vclass = getattr(vehicle, 'vehicleClass', None)
                if vclass=='car':
                    noOfCars += 1
                elif vclass=='bus':
                    noOfBuses += 1
...
def repeat():
    ...
    while(signals[currentGreen].green>0):   # while the timer of current green signal is not zero
        ...
        # Auto-adapt info: detect if current green lane is empty
        current_count = count_vehicles_in_lane(currentGreen)
        if current_count == 0 and signals[currentGreen].green > defaultMinimum + 2:
            # Current lane is empty; find lane with most traffic
            waiting = compute_waiting_counts()
            waiting_copy = waiting[:]
            waiting_copy[currentGreen] = -1  # exclude current
            max_wait = max(waiting_copy)
            if max_wait > 0:
                # Log for user awareness
                chosen = waiting_copy.index(max_wait)
                print(f"[AUTO-ADAPT INFO] Lane {currentGreen+1} is empty. Lane {chosen+1} has {max_wait} waiting vehicles. Will switch at next cycle.")
...
def count_vehicles_in_lane(direction_idx):
    """Count total vehicles (not yet crossed) in a given direction."""
    direction = directionNumbers[direction_idx]
    count = 0
    for lane in (0, 1, 2):
...
def compute_waiting_counts():
    """Return a list of 4 integers: number of vehicles waiting at red for each direction.

    A vehicle is considered waiting at red when:
    - It has not crossed the stop line (vehicle.crossed == 0)
    - The signal for its direction is not green (and not yellow)
    - Its front has reached the stop coordinate (i.e. it is positioned at/near the stop)
    """
    counts = [0, 0, 0, 0]
    # direction indices: 0:right,1:down,2:left,3:up
    # Only count when that direction's signal is red (not green/yellow)
    for d_idx in range(4):
        # Skip if the current green is this direction or yellow is active
        if currentYellow == 1 or currentGreen == d_idx:
            continue
        direction = directionNumbers[d_idx]
        for lane in (0, 1, 2):
            # collect vehicles in this lane that haven't crossed and are initialized
            lane_vehicles = []
            for vehicle in list(vehicles[direction][lane]):
                if getattr(vehicle, 'crossed', 0) != 0:
                    continue
                img = getattr(vehicle, 'currentImage', None)
                if img is None:
                    continue
                if not hasattr(vehicle, 'stop') or not hasattr(vehicle, 'x') or not hasattr(vehicle, 'y'):
                    continue
            ...
            # Determine waiting status starting from the frontmost vehicle (index 0)
            waiting_flags = [False] * len(lane_vehicles)
            # Helper to get front/back coordinate depending on direction
            def front_pos(v):
                r = v.currentImage.get_rect()
                if direction == 'right':
                    return v.x + r.width
                if direction == 'down':
                    return v.y + r.height
            ...
            # frontmost vehicle is the one with lowest index in the lane list (index 0)
            for i, v in enumerate(lane_vehicles):
                try:
                    r = v.currentImage.get_rect()
                except Exception:
                    continue
                # Check if this vehicle has reached/passed its stop coordinate
                is_at_stop = False
                if direction == 'right':
                    if v.x + r.width >= v.stop - 1:
                        is_at_stop = True
                elif direction == 'down':
                    if v.y + r.height >= v.stop - 1:
                        is_at_stop = True
                elif direction == 'left':
                    if v.x <= v.stop + 1:
                        is_at_stop = True
                ...
                if is_at_stop:
                    waiting_flags[i] = True
                else:
                    # If the vehicle behind a waiting vehicle is close (queued), mark it waiting when the vehicle ahead is waiting
                    if i > 0 and waiting_flags[i-1]:
                        # If current vehicle front has reached near the back of vehicle ahead, it's queued
                        try:
                            cur_front = front_pos(v)
                            ahead_back = back_pos(lane_vehicles[i-1])
                            if direction in ('right', 'down'):
                                if cur_front >= ahead_back - gap2:
                                    waiting_flags[i] = True
                            else:
                                # left/up: coordinates decrease when moving; compare appropriately
                                if cur_front <= ahead_back + gap2:
                                    waiting_flags[i] = True
                        except Exception:
                            pass

            # Count waiting vehicles for this lane
            counts[d_idx] += sum(1 for w in waiting_flags if w)
...
def generateVehicles():
    while(True):
        ...
        time.sleep(0.63)  # Adjusted to reduce traffic by ~5% (was 0.6)
...
class Main:
    ...
    while True:
        ...
        # compute and display waiting counts (vehicles stopped at red)
        waiting_counts = compute_waiting_counts()
        for i in range(0, noOfSignals):
            waitingText = font.render("W:" + str(waiting_counts[i]), True, black, white)
